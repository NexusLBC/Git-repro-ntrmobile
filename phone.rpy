                        ...
                        vbox:
                            ...
                            if current_app in phone_channels:
                                ...
                                for message_data in phone_channels[current_app]:
                                    ...
                                    # highlighted message
                                    $ highlight = (current_app, msg_id) in phone_pending
                                    if highlight and phone_pending[(current_app, msg_id)]:
                                        $ has_pending = True
                                        timer 0.05 repeat True action Function(phone_highlight_bump_color, current_app, msg_id, sender)
                                    ...
                                    if sender == phone_config["phone_player_name"]:
                                        ...
                                        $ text_colour = "#FFFFFF"
                                        $ msg_align = 1.0 - phone_config["message_align"]
                                        $ msg_bubble_type = 1
                                    else:
                                        $ is_player_message = False
                                        $ msg_frame = "gui/receive_frame.png"
                                        $ text_colour = get_sender_text_color(dark_mode)
                                        $ msg_bubble_type = get_bubble_type(sender)

                                    $ animated = False
                                    $ anim_direction = 0
                                    if message_kind != 1 and should_animate:
                                        $ animated = True
                                        $ anim_direction = 1 if is_player_message else -1
                                        if current_global_id not in phone_animated_global_ids[current_app]:
                                            $ phone_animated_global_ids[current_app].append(current_global_id)
                                    elif phone_user_scrolled_up.get(current_app, False):
                                        $ animated = True
                                        $ anim_direction = 1 if is_player_message else -1
                                    ...
                                    if message_kind == 1:
                                        ...
                                        text timestamp:
                                            color phone_config["timestamp_colour"]
